...
# https://gutenberg.org/cache/epub/36684/pg36684.txt

mirror_link = "http://www.mirrorservice.org/sites/ftp.ibiblio.org/pub/docs/books/gutenberg"
...
def getSomeBooks(howManyBooks, startingAt=1):
    ...
    while i > len(allTheBooks): # 54096 ceiling
        try:
            theText = strip_headers(load_etext(ii, None, mirror_link)).strip() #load the full text into theText
...
g = getSomeBooks(1, 2702)

# file = open('2701.txt', 'a')
# file.write(text)
# file.close()